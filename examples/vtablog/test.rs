...
fn update() -> rusqlite::Result<()> {
    ...
    let expected = patch_output(indoc! {r#"
        ...
        commit(tab=100, transaction=101)
        drop_transaction(tab=100, transaction=101)
        <M best_index(tab=100, index_info=IndexInfo { constraints: [IndexInfoConstraint { column: -1, op: Eq, usable: true, rhs: Err(Sqlite(12)), argv_index: None, omit: false }], order_by: [], index_num: 0, index_str: None, order_by_consumed: false, estimated_cost: 5e98 })
...
fn delete() -> rusqlite::Result<()> {
    ...
    let expected = patch_output(indoc! {r#"
        ...
        commit(tab=100, transaction=101)
        drop_transaction(tab=100, transaction=101)
        <M best_index(tab=100, index_info=IndexInfo { constraints: [IndexInfoConstraint { column: 0, op: Eq, usable: true, rhs: Err(Sqlite(12)), argv_index: None, omit: false }], order_by: [], index_num: 0, index_str: None, order_by_consumed: false, estimated_cost: 5e98 })
