...
pub fn eponymous_only_module<'vtab, T: VTab<'vtab>>() -> Module<'vtab, T> {
    Module {
...
unsafe extern "C" fn vtab_connect<'vtab, T: VTab<'vtab> + 'vtab>(
    db: *mut ffi::sqlite3,
    module: *mut c_void,
    argc: i32,
    argv: *const *const i8,
...
impl<'vtab, T: VTab<'vtab>> Drop for ModuleHandle<'vtab, T> {
    fn drop(&mut self) {
